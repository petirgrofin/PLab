...
export function VennDiagramDragNDrop({
  ...
  // Choose config based on screen size
  const effectiveConfig = isExtraSmallScreen
    ? EXTRA_SMALL_VENN_CONFIG
    : isSmallScreen
    ? SMALL_VENN_CONFIG
    : config;
  ...
  const controlled =
    Array.isArray(selectedRegions) && typeof setSelectedRegions === "function";
  ...
  const shapes = useMemo(
    () => constructVennDiagram(effectiveConfig),
    [effectiveConfig]
  );
  ...
  const labelCoords = useMemo(
    () => getLabelCoords(effectiveConfig),
    [effectiveConfig]
  );
  ...
  const handleSelect = useCallback(
    (regionId) => {
      ...
      updateSel(
        sel.includes(regionId)
          ? sel.filter((r) => r !== regionId)
          : [...sel, regionId]
      );
    },
  ...
  const handleDragEnd = ({ active, over }) => {
    setActiveId(null);
    const map = !over
      ? { ...assignments, [active.id]: null }
      : { ...assignments, [active.id]: over.id };
    setExerciseResponse({
      vennDiagramDND: true,
      response: computeExerciseResponse(map),
    });
    setAssignments(map);
  };

  const { width, height } = effectiveConfig;
      ...
      <div className="flex flex-col items-center justify-center">
        ...
        <div className="flex justify-center items-center overflow-x-auto">
          <svg width={width} height={height} className="select-none">
            ...
            <foreignObject
              x={
                effectiveConfig === DEFAULT_VENN_CONFIG
                  ? 100
                  : effectiveConfig === SMALL_VENN_CONFIG
                  ? 60
